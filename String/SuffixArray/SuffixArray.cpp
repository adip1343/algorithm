...
vector <int> p(MAXN*2);		//Position of ith suffix
vector <int> c(MAXN*2);		//Comparision Class of ith suffix
vector <int> lcp(MAXN*2);	//longest common prefix of suffixes
...
string LongestCommonSubstring(string S,string T){
    ...
    for(int i=3;i<n;i++){
        ...
        if(i1<m && i2>m && len<lcp[i]){
            len = lcp[i];
            index = i1;
        }
    }
    string s = S.substr(index,len);
    cout<<s<<endl;
    return s;
}
